...
def func(message):
    re = requests.get('http://localhost:5000/api/chat_id_for_registration').json()
    chat_id = re['chat_id']
...
def end_authorization(message, login):
    print(requests.post('http://localhost:5000/api/create_new_profile',
        ...
        if 0 < int(message.text) <= 10:
            if int(message.text) >= 5:
                ...
                if re['error'] == 'No error':
                    ...
                    print(js_dik)
                    requests.post(f'http://localhost:5000/api/new_genres/{message.chat.id}',
...
def is_bookmark(message, content):
    if message.text == "Добавить в закладки":
        print(requests.post(f'http://localhost:5000/api/add_bookmark/{message.chat.id}',
                      json={
